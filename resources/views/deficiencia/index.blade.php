...
<div class="aluno-page">
  <div class="aluno-card">
    ...
    <div class="header-flex">
      <h1 class="aluno-title">Tipos de Deficiência</h1>
    ...
    <div class="table-responsive">
      <table class="custom-table">
        ...
        <tbody>
          @forelse($deficiencias as $index => $deficiencia)
            <tr>
              <td style="font-weight:700; color:#64748b;">#{{ $index + 1 }}</td>
              <td style="font-weight:600; color:#1e293b;">{{ $deficiencia->nome }}</td>
              <td style="text-align:right;">
                <button class="btn-action btn-editar"
                  onclick="abrirModalEditar({{ $deficiencia->id }}, '{{ addslashes($deficiencia->nome) }}')">
                  ✏️ Editar
                </button>
                <button class="btn-action btn-excluir"
                  onclick="abrirConfirmacao({{ $deficiencia->id }}, '{{ addslashes($deficiencia->nome) }}')">
                  🗑️ Excluir
                </button>
              </td>
            </tr>
          @empty
            <tr>
              <td colspan="3" class="tabela-vazia">Nenhum tipo de deficiência cadastrado.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
...
<div class="modal-backdrop-custom" id="modalBackdrop">
  <div class="modal-box">
    <h2 id="modalTitulo">Novo Tipo de Deficiência</h2>
    <form id="formCriar" action="{{ route('deficiencias.store') }}" method="POST">
      @csrf
      <div class="mb-3">
        <label class="form-label fw-semibold mb-2">Nome da deficiência</label>
...
<div class="modal-backdrop-custom" id="modalBackdropEditar">
  <div class="modal-box">
    <h2>Editar Tipo de Deficiência</h2>
...
<div class="confirm-backdrop" id="confirmBackdrop">
  <div class="confirm-modal">
    ...
    <h3 class="confirm-title">Excluir deficiência?</h3>
...
<script>
  ...
  function abrirModalEditar(id, nome) {
    ...
    document.getElementById('formEditar').action = '/deficiencias/' + id;
  ...
  function abrirConfirmacao(id, nome) {
    document.getElementById('confirmTexto').textContent =
      'A deficiência "' + nome + '" será excluída permanentemente.';
    document.getElementById('formExcluir').action = '/deficiencias/' + id;
