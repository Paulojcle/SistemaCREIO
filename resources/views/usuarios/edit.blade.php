...
<div class="escola-page">
    <div class="escola-card">
        ...
        <h1 class="escola-title">Editar Usuário</h1>
        ...
        <form action="{{ route('usuarios.update', $usuario->id) }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-12 col-md-6">
                    ...
                    <input type="text" name="firstName" value="{{ old('firstName', $usuario->firstName) }}"
                ...
                <div class="col-12 col-md-3">
                    <label class="form-label">Nova Senha <small style="color:#94a3b8;">(deixe em branco para manter)</small></label>
                        ...
                        @foreach($perfis as $perfil)
                            <label style="display:flex; align-items:center; gap:6px; cursor:pointer;">
                                <input type="checkbox" name="perfis[]" value="{{ $perfil->id }}"
                                    {{ in_array($perfil->id, old('perfis', $perfisDoUsuario)) ? 'checked' : '' }}>
                                {{ $perfil->nome }}
                            </label>
                        @endforeach
                    </div>
                    ...
                    <select name="profissional_id" class="form-control">
                        ...
                        @foreach($profissionais as $profissional)
                            <option value="{{ $profissional->id }}"
                                {{ old('profissional_id', $usuario->profissional?->id) == $profissional->id ? 'selected' : '' }}>
