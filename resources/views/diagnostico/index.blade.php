...
<div class="aluno-page">
  <div class="aluno-card">
    ...
    <div class="header-flex">
      <h1 class="aluno-title">Tipos de Diagnóstico</h1>
    ...
    <div class="table-responsive">
      <table class="custom-table">
        ...
        <tbody>
          @forelse($diagnosticos as $index => $diagnostico)
            <tr>
              <td style="font-weight:700; color:#64748b;">{{ $index + 1 }}</td>
              <td style="font-weight:600; color:#1e293b;">{{ $diagnostico->nome }}</td>
              <td style="text-align:right;">
                <button class="btn-action btn-editar"
                  onclick="abrirModalEditar({{ $diagnostico->id }}, '{{ addslashes($diagnostico->nome) }}')">
                  ✏️ Editar
                </button>
                <button class="btn-action btn-excluir"
                  onclick="abrirConfirmacao({{ $diagnostico->id }}, '{{ addslashes($diagnostico->nome) }}')">
                  🗑️ Excluir
                </button>
              </td>
            </tr>
          @empty
            <tr>
              <td colspan="3" class="tabela-vazia">Nenhum diagnóstico cadastrado.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
...
<div class="modal-backdrop-custom" id="modalBackdrop">
  <div class="modal-box">
    <h2 id="modalTitulo">Novo Tipo de Diagnóstico</h2>
    <form id="formCriar" action="{{ route('diagnosticos.store') }}" method="POST">
      @csrf
      <div class="mb-3">
        <label class="form-label fw-semibold mb-2">Nome do diagnóstico</label>
        <input type="text" name="nome"
          class="form-control soft-input"
          placeholder="Ex: Síndrome de Asperger..."
          required>
      </div>
...
<div class="modal-backdrop-custom" id="modalBackdropEditar">
  <div class="modal-box">
    <h2>Editar Tipo de Diagnóstico</h2>
...
<div class="confirm-backdrop" id="confirmBackdrop">
  <div class="confirm-modal">
    ...
    <h3 class="confirm-title">Excluir diagnóstico?</h3>
...
<script>
  ...
  function abrirModalEditar(id, nome) {
    ...
    document.getElementById('formEditar').action = '/diagnosticos/' + id;
  ...
  function abrirConfirmacao(id, nome) {
    document.getElementById('confirmTexto').textContent =
      'O diagnóstico "' + nome + '" será excluído permanentemente.';
    document.getElementById('formExcluir').action = '/diagnosticos/' + id;
